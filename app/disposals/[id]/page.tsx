...
interface DisposalDetailPageProps {
  params: {
    id: string;
  };
}

export default function DisposalDetailPage({ params }: DisposalDetailPageProps) {
  ...
  React.useEffect(() => {
    if (feedbackOpen) {
      fetch('http://localhost:8000/users?role=admin|manager&permission=disposals', {
  ...
  return (
    <ProtectedRoute>
      <MainNav />
      <Box p={3}>
        <DisposalDetail disposalId={disposalId} />
